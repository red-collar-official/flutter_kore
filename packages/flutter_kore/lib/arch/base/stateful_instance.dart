...
import 'package:flutter_kore/arch/base/kore_instance.dart';

import 'event_bus.dart';
import 'flutter_kore_app.dart';
import 'store.dart';
...
///
/// If used you need to call [initializeStatefulInstance] in [BaseKoreInstance.initialize] call
/// And call [disposeStore] in [BaseKoreInstance.dispose] call
///
/// ```dart
...
/// }
/// ```
mixin StatefulKoreInstance<State, Input> on KoreInstance<Input> {
  ...
  ///   final posts = state.posts.unwrap().toList();
  /// }
  /// ```
  ...
  ///
  /// ```dart
  /// Stream<StatefulData<List<Post>>?> get postsStream => useLocalInstance<PostsInteractor>().updates((state) => state.posts);
  /// ```
  ...
  ///
  ///     final response = await app.apis.posts.getPosts().execute();
  ///
  ...
  ///
  /// if [savedStateObject] is not empty listens to state updates and puts it to cache using [KoreApp.cachePutDelegate]
  ...
  void _subscribeToStoreUpdates() {
    ...
    _storeSaveSubscription = _store.stream.listen((_) async {
      ...
      await KoreApp.cachePutDelegate(stateId, json.encode(savedStateObject));
    });
  }
