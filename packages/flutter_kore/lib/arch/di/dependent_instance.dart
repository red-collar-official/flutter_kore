...
class DependentKoreInstanceConfiguration extends KoreInstanceConfiguration {
...
///
/// If used you need to call [initializeDependencies] in [BaseKoreInstance.initialize] call
/// And call [disposeDependencies] in [BaseKoreInstance.dispose] call
///
/// Example:
///
/// ```dart
...
/// }
/// ```
mixin DependentKoreInstance<Input> on KoreInstance<Input> {
  /// Local instances
  ///
  /// Does not hold singleton instances
  ...
  Future<void> _addInstancesAsync() async {
    final asyncDeps = getFullConnectorsList().where(
      (element) => element.isAsync && !element.isLazy,
    );

    await Future.wait(asyncDeps.map(_addAsyncInstance));
  ...
  T useLocalInstance<T extends BaseKoreInstance>({int index = 0}) {
    if (_instances[T] == null) {
      throw IllegalStateException(message: 'Instance $T is not connected.');
    }
