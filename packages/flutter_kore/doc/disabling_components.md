...
You can disable most of the components of flutter_kore entirely.
...
It is also true that you can bypass all components and and use them as state management. You can also safely call `Apis` (if you use them) in view models because view models mix the `ApiCaller` mixin (more information about `ApiCaller` [here](./custom_instances.md)).

This way you can implement your own domain design.

It is also useful for small projects because you can just use state management and split it into `InstancePart` if the code starts to grow (more information about `InstancePart` [here](./instance_part.md)).
